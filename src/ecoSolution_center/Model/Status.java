package ecoSolution_center.Model;

public class Status {


    public final static String arrivedInCenter = "Arrived in center";
    public final static String inWashing = "In washing";
    public final static String inDrying = "In drying";
    public final static String inIroning = "In ironing";
    public final static String readyForTransport = "Ready for transport";
    public final static String inTransportToShop = "In transport to shop";


}
